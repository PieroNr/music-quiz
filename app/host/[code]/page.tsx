...
export default function HostRoomPage({ params }: { params: Promise<{ code: string }> }) {
    ...
    // ✅ nouveau : pour éviter plusieurs /round/end
    ...
    useEffect(() => {
        ...
        channel.bind("round-started", (payload: RoundPayload) => {
            ...
            isEndingRoundRef.current = false; // ✅ on déverrouille pour la nouvelle manche
    ...
    async function playUrl(url: string, segment: "Q" | "A" | "B" | "C" | "D") {
        ...
        // ✅ on arrête proprement ce qui jouait avant
        try {
            el.pause();
        } catch {}
        el.currentTime = 0;
    ...
    // fin de manche + reveal audio
    async function endRoundAndReveal() {
        if (!code || !round) return;

        // ✅ si on est déjà en train de terminer la manche, on sort
        ...
        try {
            ...
            // Si le serveur dit 409 (déjà terminé), on ignore simplement
            if (res.status === 409) {
                return;
            }
            ...
            if (round.answerUrl) {
                await playUrl(round.answerUrl, "Q");
            }
        } catch (e) {
            setError(e instanceof Error ? e.message : "Erreur inconnue");
            // si tu veux autoriser un retry manuel en cas d’erreur réseau, tu peux remettre :
    ...
    if (!code)
        return (
            <main className="relative min-h-screen bg-[#050509] text-white overflow-hidden">
                ...
                />
                <div className="relative z-10 p-8">Chargement…</div>
            </main>
        );
